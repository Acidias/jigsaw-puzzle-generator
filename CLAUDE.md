...
- `Sources/Models/` - Data models:
  ...
  - `ChatToolDefinitions` - Tool schemas for LLM function calling (8 read-only tools, generates both Claude and OpenAI wire formats)
...
- `Sources/Services/`
  ...
  - `ChatToolExecutor` - Maps LLM tool calls to app data (ModelState, DatasetState, ModelStore.buildTrainingReport). 8 read-only tools: list/get models, datasets, presets, training reports, model comparison
...
- **AI Chat assistant**: In-app LLM chat with tool use. Supports Claude API and OpenAI API (switchable). 8 read-only tools give the LLM access to models, datasets, presets, training reports, and model comparison. SSE streaming via URLSession.bytes(for:). API keys stored in UserDefaults. Sidebar: AI Tools > AI Chat. Foundation for future autopilot.
