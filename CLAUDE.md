...
- `Sources/Models/` - Data models:
  ...
  - `AutoMLSearchSpace` - Search space types: SearchableParam enum (13 hyperparameters), SearchDimension (intRange/floatRange/categorical mapping to Optuna suggest), OptimisationMetric (validAccuracy/validLoss/recallAtP70), AutoMLConfiguration struct
  - `AutoMLTrial` - Trial result: TrialState (complete/pruned/fail), params, value, duration, accuracy
  - `AutoMLStudy` - Persisted AutoML study entity (ObservableObject): name, configuration, trials, status lifecycle (configured/running/completed/cancelled/failed), bestModelID
  - `AutoMLStudyManifest` - Codable DTO for study persistence (reverts running to configured on load for crash recovery)
  - `AutoMLState` - Central state for AutoML (studies list, running status, live trials, current trial progress with live metrics)
...
- `Sources/Services/`
  ...
  - `AutoMLStudyStore` - Persistence layer for AutoML studies: save/load/delete to ~/Library/Application Support/JigsawPuzzleGenerator/studies/
  - `AutoMLScriptGenerator` - Generates complete Optuna Python training scripts with SQLite crash recovery, structured stdout protocol, dataset caching across trials, best model evaluation
  - `AutoMLRunner` - Orchestrates local and cloud AutoML training: subprocess management, stdout parsing (AUTOML_START/EPOCH/TRIAL_COMPLETE/PRUNED/COMPLETE), cloud checkpoint polling (60s), result import as SiameseModel
...
- **Disk layout**: `projects/<project-uuid>/manifest.json` + `images/<image-uuid>/source.<ext>` + `cuts/<cut-uuid>/<image-uuid>/pieces/*.png` + `lines.png`. Datasets: `datasets/<dataset-uuid>/manifest.json` + `{train,test,valid}/<category>/pair_NNNN_{left,right}.png` + `labels.csv`. Presets: `presets/<preset-uuid>/manifest.json`. Models: `models/<model-uuid>/manifest.json` + `metrics.json` + `model.mlpackage/` + `training/` (train.py, requirements.txt, best_model.pth). Studies: `studies/<study-uuid>/manifest.json` + `training/` (automl_train.py, requirements.txt, study.db, optuna_results.json, best_model.pth, best_metrics.json). Chat: `chat/session.json`
...
- **AutoML (Optuna)**: Automated hyperparameter search using Optuna. Select a base architecture preset + dataset, toggle which parameters to search (13 searchable params across architecture/training/mode sections), configure trials/pruning/metric. Runs as a single subprocess (local or cloud SSH) with structured stdout for live progress. Studies are first-class persisted entities at ~/Library/Application Support/JigsawPuzzleGenerator/studies/. SQLite storage enables crash recovery and resume. Best trial auto-imported as SiameseModel. Cloud training uses 60s checkpoint polling via SCP. Mutual exclusion with normal training (both check each other's running state). Sidebar: AI Tools > AutoML + individual study entries.
