...
- `Sources/Models/` - Data models:
  ...
  - `ChatToolDefinitions` - Tool schemas for LLM function calling (11 tools including 3 image-returning, generates both Claude and OpenAI wire formats)
...
- `Sources/Services/`
  ...
  - `ChatToolExecutor` - Maps LLM tool calls to app data (ModelState, DatasetState, AppState, ModelStore.buildTrainingReport). 11 tools: list/get models, datasets, presets, training reports, model comparison, plus 3 image-returning tools (get_sample_pairs, list_projects, get_piece_images). Returns (String, [ToolResultImage]) tuple for image support.
