...
- `Sources/Models/` - Data models:
  ...
  - `ChatState` - AI chat assistant state (messages, provider/model selection, streaming status, credential store via UserDefaults)
  - `ChatToolDefinitions` - Tool schemas for LLM function calling (15 tools: 11 read-only + 4 write tools, generates both Claude and OpenAI wire formats)
...
- `Sources/Services/`
  ...
  - `LLMService` - Multi-provider LLM API integration (Claude + OpenAI) with SSE streaming, system prompt, tool call loop
  - `ChatToolExecutor` - Maps LLM tool calls to app data (ModelState, DatasetState, AppState, ModelStore.buildTrainingReport). 15 tools: list/get models, datasets, presets, training reports, model comparison, 3 image-returning tools (get_sample_pairs, list_projects, get_piece_images), plus 4 write tools (create_model, read_training_script, update_training_script, start_training). Async execute() method. Returns (String, [ToolResultImage]) tuple for image support.
...
- **AI Chat assistant**: In-app LLM chat with tool use. Supports Claude API and OpenAI API (switchable). 15 tools: 11 read-only (list/get models, datasets, presets, training reports, model comparison, vision tools) plus 4 write tools (create_model, read_training_script, update_training_script, start_training) giving the LLM full creative control over the training pipeline. Vision tools return base64-encoded images alongside rich metadata. Claude: images embedded in tool_result content blocks. OpenAI: images injected as follow-up user messages with data URI image_url parts. Image thumbnails displayed in chat UI inside tool result DisclosureGroups. SSE streaming via URLSession.bytes(for:). API keys stored in UserDefaults. Chat session persisted to ~/Library/Application Support/JigsawPuzzleGenerator/chat/session.json (auto-saves on message changes, loaded on app launch). Sidebar: AI Tools > AI Chat.
